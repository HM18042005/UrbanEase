...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const [totalServices, activeBookings, totalEarnings, avgRating] = await Promise.all([
      ...
      Booking.countDocuments({ provider: providerId, status: { $in: ['confirmed', 'in-progress'] } }),
      Booking.aggregate([
...
exports.getMessages = async (req, res) => {
  try {
    ...
    await Message.populate(conversations, { path: '_id', select: 'name email' });
...
exports.updateAvailability = async (req, res) => {
  try {
    // This would update provider availability settings
